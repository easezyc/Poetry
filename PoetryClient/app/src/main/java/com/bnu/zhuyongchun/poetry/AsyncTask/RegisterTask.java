...
public class RegisterTask extends AsyncTask<String, Integer, Boolean>{
    ...
    protected Boolean doInBackground(String... params) {//两个参数邮箱和密码
        ...
        try{
            Connection.Response res = Jsoup.connect("http://202.112.88.39:8080/PoetryServer/servlet/UserRegisterServlet").data("uid",email).data("password",password).data("uname",username)
