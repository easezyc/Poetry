 ...
 */
public class WriteverseTask extends AsyncTask<String, Integer, String>{
    ...
    public WriteverseTask(OnLoginedListener listener) {
        super();
        mListener = listener;
    }
    ...
    protected String doInBackground(String... params) {//两个参数邮箱和密码
        ...
        String verse=params[1];
        String email= User.getUser().getUseremail();
        try{
            Connection.Response res = Jsoup.connect("http://202.112.88.39:8080/PoetryServer/servlet/InsertVerseSvt").data("pname",name).data("pcontent",verse).data("uid",email)//修改uid,verse
