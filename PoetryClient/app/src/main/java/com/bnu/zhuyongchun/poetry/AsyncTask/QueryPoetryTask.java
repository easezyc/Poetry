 ...
 */
public class QueryPoetryTask extends AsyncTask<String, Integer, Poetry> {
    ...
    public QueryPoetryTask(OnLoginedListener listener) {
        super();
        mListener = listener;
    }
    ...
    protected Poetry doInBackground(String... params) {//两个参数邮箱和密码
        String pid=params[0];
        Poetry poetry=new Poetry();//("aa","李白","床前明月光，疑是地上霜。举头望明月，低头思故乡。");
        try{
            Connection.Response res = Jsoup.connect("http://202.112.88.39:8080/PoetryServer/servlet/ShowPoetrySvt")
                    .data("id",pid)
